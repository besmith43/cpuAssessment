...
public class Class1
{
    ...
    public static async Task<bool> FindIPAsync(ByteIP ipCanidate, ByteIP[] ipPool)
    {
        var tasks = new List<Task<bool>>();
        for (int i = 0; i < 4; i++)
        {
            // Console.WriteLine($"FindIPAsync Iteration: { i }");
            var tempArray = Chunkify(ipPool, 4, i+1);
            tasks.Add(Task.Run(async () => {
                return await _FindIPAsync(ipCanidate, tempArray);
            }));
        }
        ...
        {
            Console.WriteLine($"Async failed with error message: { ex.Message }");
        }
        ...
        {
            Console.WriteLine("The 4 Async Tasks failed");
            return false;
        }
    }

    private static async Task<bool> _FindIPAsync(ByteIP ipCanidate, ByteIP[] ipPool)
    {
        bool containsFlag = false;
    ...
    public static ByteIP[] Chunkify(ByteIP[] array, int totalNumChunks, int interation)
    {
        int totalSize = array.Length;
        int chunkSize = totalSize / totalNumChunks;

        // Console.WriteLine($"Total Size: { totalSize }");
        // Console.WriteLine($"Chunk Size: { chunkSize }");
        // Console.WriteLine($"Interation: { interation }");
        // Console.WriteLine($"total number of chunks: { totalNumChunks }");

        int startIndex = (chunkSize * (interation - 1));

        // Console.WriteLine($"Starting Index: { startIndex }");

        var slice = new ByteIP[chunkSize];
        Array.Copy(array, startIndex, slice, 0, chunkSize);
        return slice;
    }
}
